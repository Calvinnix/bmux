...
Everything beyond [installing](README.md). The prefix is **ctrl-b** (tmux default);
press `prefix ?` for searchable in-app help, and every new tab shows the cheatsheet.
...
Ads and trackers are blocked with full filter lists (EasyList + EasyPrivacy, via
Ghostery's adblocker engine), including cosmetic element hiding — blocked requests show
as the `⊘ n` counter in the status bar. Lists download on first launch, are cached in the
profile directory, and refresh weekly; until the first download completes (or if it
fails), a built-in list of ~55 common ad/tracking domains covers the basics. Every
request also carries `DNT: 1` and `Sec-GPC: 1` headers.

Set `blockTrackers: false` to disable (also toggleable from the palette), `blockExtra`
to block additional domains, and drop a `filters.txt` with adblock-syntax filter lines
(one per line, `!` comments) next to `config.json` for custom rules.
...
profile directory; `BMUX_ADBLOCK=local` skips the filter-list download and uses only
`filters.txt` (the test suite runs this way to stay off the network).
